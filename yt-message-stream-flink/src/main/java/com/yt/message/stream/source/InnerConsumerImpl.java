...
import com.yt.message.stream.source.reader.MessageViewExt;
import com.yt.message.stream.util.UtilAll;
...
public class InnerConsumerImpl implements InnerConsumer {
    ...
    public InnerConsumerImpl(Configuration configuration) {
        ...
        String accessKey = configuration.get(RocketMQSourceOptions.OPTIONAL_ACCESS_KEY);
        String secretKey = configuration.get(RocketMQSourceOptions.OPTIONAL_SECRET_KEY);
        ...
        String groupId = configuration.get(RocketMQSourceOptions.CONSUMER_GROUP);
        String endPoints = configuration.get(RocketMQSourceOptions.ENDPOINTS);
        Integer pullThreadNums = configuration.get(RocketMQSourceOptions.PULL_THREAD_NUM);
        ...
        this.consumer.setPullThreadNums(pullThreadNums);
...
//        int threadNum =
//                configuration.get(RocketMQSourceOptions.PULL_THREADS_NUM, processors);
        int threadNum = 4;
    ...
    public String getConsumerGroup() {
        return configuration.get(RocketMQSourceOptions.CONSUMER_GROUP);
    }
