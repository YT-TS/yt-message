...
package com.yt.message.stream.source.reader;

import com.yt.message.stream.common.config.RocketMQOptions;
...
public class RocketMQSplitReader<T> implements SplitReader<MessageView, RocketMQSourceSplit> {
    ...
    public RocketMQSplitReader(
        ...
        this.commitOffsetsOnCheckpoint =
                configuration.get(RocketMQOptions.COMMIT_OFFSETS_ON_CHECKPOINT);
    }
    ...
    public RecordsWithSplitIds<MessageView> fetch() {
        ...
        try {
            Duration duration =
                    Duration.ofMillis(this.configuration.get(RocketMQOptions.POLL_TIMEOUT));
            ...
            for (MessageView messageView : messageExtList) {
                ...
                if (this.configuration.get(RocketMQSourceOptions.GLOBAL_DEBUG_MODE)) {
                    LOG.info(
