...
import com.yt.message.stream.source.InnerConsumer;
import com.yt.message.stream.source.InnerConsumerImpl;
import com.yt.message.stream.source.RocketMQSourceOptions;
import com.yt.message.stream.source.enumstate.allocate.AllocateStrategy;
import com.yt.message.stream.source.enumstate.allocate.AllocateStrategyFactory;
...
public class RocketMQSourceEnumerator
    ...
    public RocketMQSourceEnumerator(
        ...
        this.groupId = configuration.get(RocketMQSourceOptions.CONSUMER_GROUP);
    ...
    private Set<MessageQueue> requestServiceDiscovery() {
        Set<String> topicSet =
                Sets.newHashSet(
                        configuration.get(RocketMQSourceOptions.TOPIC)
    ...
    private SourceSplitChangeResult initializeSourceSplits(SourceChangeResult sourceChangeResult) {
        Set<MessageQueue> increaseSet = sourceChangeResult.increaseSet();
        ...
        return new SourceSplitChangeResult(increaseSplitSet, sourceChangeResult.decreaseSet());
    }
     ...
     */
        @VisibleForTesting
        private record SourceChangeResult(Set<MessageQueue> increaseSet, Set<MessageQueue> decreaseSet) {
